> [!IMPORTANT]
> **Reference-fork archival prepared 2026-08-16; repository-settings authorization is still required to make this repository read-only.** The authoritative upstream is [`headroomlabs-ai/headroom`](https://github.com/headroomlabs-ai/headroom), licensed under the [Apache License 2.0](LICENSE). This is not original independent work; `pc-style`'s historical delta was a narrow Amp plugin and `headroom wrap amp` integration with tests and local-proxy URL validation. That delta was not present in upstream's default branch at review, so this notice does **not** claim it was merged upstream. Use and report issues to upstream.
>
> The Amp integration accepts only `localhost`, `127.0.0.1`, and IPv6 loopback (`::1`) proxy URLs, rejecting remote hosts to avoid forwarding Amp tool data to an arbitrary proxy. This is defense in depth, not a security warranty. The complete Git history and both retained branches (`main` and `pcs/amp-support-a216`) remain available for provenance.
...
- **Agent wrap** — `headroom wrap claude|codex|cursor|aider|copilot|amp` in one command
...
| Amp         | ✅              | installs `.amp/plugins` plugin   |
...
- **CLI/tool slices**: `headroom/providers/claude`, `copilot`, `codex`, `openclaw`, `amp`
