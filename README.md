> [!IMPORTANT]
> **Historical reference fork reviewed 2026-08-16; not archive-ready.** The authoritative upstream is [`headroomlabs-ai/headroom`](https://github.com/headroomlabs-ai/headroom), licensed under the [Apache License 2.0](LICENSE). This is not original independent work; `pc-style`'s historical delta is a narrow Amp plugin and `headroom wrap amp` integration with tests and local-proxy URL validation. That delta was not present in upstream's default branch at review and must be accepted upstream or otherwise dispositioned before this fork is archived. Use and report unrelated issues to upstream.
>
