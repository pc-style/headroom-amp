> **Archived reference fork — 2026-08-16.** The authoritative upstream is [`headroomlabs-ai/headroom`](https://github.com/headroomlabs-ai/headroom). Headroom is licensed under the [Apache License 2.0](LICENSE). This fork is not original independent work; `pc-style`'s historical delta was a narrowly scoped Amp plugin and `headroom wrap amp` integration, including tests and local-proxy URL validation. The fork is no longer maintained. At archival inspection, that Amp-specific delta was not present in upstream's default branch, so this notice does **not** claim it was merged upstream. Use and report issues to upstream.
>
